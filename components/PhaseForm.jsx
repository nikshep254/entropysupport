    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      {questions.map(q => (
        <div key={q.id}>
          <Label>{q.label}</Label>
          {q.help && (
            <p style={{ fontSize: '9px', color: '#888', marginBottom: '7px', lineHeight: 1.6 }}>
              {q.help}
            </p>
          )}
          ...
          {q.type === 'range' && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <span style={{ fontSize: '9px', color: '#888', minWidth: '22px' }}>{q.min}</span>
              <input
                type="range"
              ...
              <span style={{ fontSize: '9px', color: '#888', minWidth: '22px', textAlign: 'right' }}>{q.max}</span>
            </div>
          )}
